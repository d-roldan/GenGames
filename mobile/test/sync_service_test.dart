...
void main() {
  ...
  test('acknowledges confirmed events after connectivity returns', () async {
    ...
    final client = MockClient((request) async {
      if (request.url.path.endsWith('/installations')) return http.Response(jsonEncode({'registered': true, 'installation_uuid': await db.installationId}), 200);
      return http.Response(jsonEncode({'accepted': [queuedId]}), 200);
    });
    final result = await SyncService(database: db, api: ApiClient('http://local/api/v1', client: client)).synchronize();
  ...
  test('server timeout keeps queue without throwing', () async {
    ...
    expect(await SyncService(database: db, api: ApiClient('http://local/api/v1', client: client)).synchronize(), isFalse);

...
void main() {
  ...
  test('validates checksum and atomically installs a pack', () async {
    ...
    final service = ContentService(
        database: db,
        api: ApiClient('http://local'),
        client: MockClient((_) async => http.Response.bytes(bytes, 200)));
    final installed = await service.download({
      'content_id': 'farm',
      'version': 1,
      'checksum': sha256.convert(bytes).toString(),
      'download_url': 'http://local/farm'
    }, directory: dir);
    expect(installed, isTrue);
    expect(
        await File('${dir.path}${Platform.pathSeparator}farm.pack')
            .readAsString(),
        'animal pack');
  ...
  test('bad checksum preserves an existing pack', () async {
    final db = await LocalDatabase.open(path: inMemoryDatabasePath);
    final dir = await Directory.systemTemp.createTemp('gengames_content_');
    final existing = File('${dir.path}${Platform.pathSeparator}farm.pack');
    await existing.writeAsString('good');
    final service = ContentService(
        database: db,
        api: ApiClient('http://local'),
        client: MockClient((_) async => http.Response('corrupt', 200)));
    expect(

...
class _CatGameScreenState extends State<CatGameScreen>
  ...
  void react(String interaction) {
    ...
    widget.services.audio.playCat(interaction);
    widget.services.analytics.track('cat_interaction',
        gameId: 'cat_game', metadata: {'interaction': interaction});
    setState(() => mood = interaction == 'bed'
        ? 'sleeping'
        : interaction == 'food'
            ? 'eating'
            : 'happy');
            ...
            children: [
                ...
                child: Center(
                  child: AnimatedBuilder(
                    ...
                    builder: (_, __) => Transform.rotate(
                      angle: mood == 'sleeping'
                          ? 0
                          : math.sin(animation.value * math.pi * 2) * .035,
                      child: SizedBox(
                        width: 330,
                        height: 360,
                        child: Stack(
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    _CatObject(
                        icon: '🐟', label: 'food', onTap: () => react('food')),
                    _CatObject(
                        icon: '🧶', label: 'toy', onTap: () => react('toy')),
                    _CatObject(
                        icon: '🛏️', label: 'bed', onTap: () => react('bed')),
                  ],
                ),
              ),
            ],
...
class _CatObject extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Draggable<String>(
      ...
      childWhenDragging: const SizedBox(width: 76, height: 76),
      child: InkWell(
...
class CatPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final ear2 = Path()
      ...
      ..lineTo(173, 61)
      ..close();
    canvas.drawPath(ear2, fur);
    canvas.drawArc(Rect.fromLTWH(222, 180, 95, 125), -1.3, 3.2, false, dark);
    if (mood == 'sleeping') {
      canvas.drawArc(Rect.fromLTWH(126, 84, 28, 15), 0, math.pi, false, dark);
      canvas.drawArc(Rect.fromLTWH(177, 84, 28, 15), 0, math.pi, false, dark);
    } else {
      canvas.drawCircle(
          const Offset(141, 94), 7, Paint()..color = Colors.black);
      canvas.drawCircle(
          const Offset(190, 94), 7, Paint()..color = Colors.black);
    }
    canvas.drawCircle(
        const Offset(165, 118), 5, Paint()..color = Colors.pinkAccent);
    ...
    if (mood == 'sleeping') {
      final text = TextPainter(
        ...
        ..layout();
      text.paint(canvas, const Offset(245, 50));
    }
  }

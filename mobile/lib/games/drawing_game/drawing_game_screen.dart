...
class _DrawingGameScreenState extends State<DrawingGameScreen> {
  ...
  void addPoint(Offset? point) {
    if (!started) {
      started = true;
      widget.services.analytics
          .track('drawing_started', gameId: 'drawing_game');
    }
          ...
          Container(
              ...
              child: Wrap(
                  ...
                  children: [
                    for (final option in [
                      ...
                      GestureDetector(
                          ...
                          child: AnimatedContainer(
                              ...
                              width: color == option ? 58 : 50,
                              height: color == option ? 58 : 50,
                              decoration: BoxDecoration(
                                  color: option,
                    ...
                    IconButton(
                        onPressed: () =>
                            setState(() => width = width == 10 ? 24 : 10),
                        icon: Icon(width == 10 ? Icons.brush : Icons.circle)),
                  ])),
        ])),
      );
}
...
class DrawStroke {
  const DrawStroke(this.point, this.color, this.width);
  final Offset? point;
  final Color color;
  final double width;
}
...
class DrawingPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    for (var i = 0; i < strokes.length - 1; i++) {
      final a = strokes[i], b = strokes[i + 1];

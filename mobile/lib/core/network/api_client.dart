...
class ApiClient {
  ...
  Future<Map<String, Object?>> post(String path, Object body) async {
    final response = await client.post(Uri.parse('$baseUrl$path'), headers: {'content-type': 'application/json'}, body: jsonEncode(body)).timeout(const Duration(seconds: 8));
  ...
  Future<List<Object?>> getList(String path) async {
    final response = await client.get(Uri.parse('$baseUrl$path')).timeout(const Duration(seconds: 12));
    if (response.statusCode < 200 || response.statusCode >= 300) throw ApiException(response.statusCode);

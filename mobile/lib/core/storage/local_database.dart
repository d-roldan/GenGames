...
class LocalDatabase {
  ...
  static Future<LocalDatabase> open({String? path}) async {
    ...
    final database =
      ...
      await db.execute(
          'CREATE TABLE sync_queue (id TEXT PRIMARY KEY, event_type TEXT NOT NULL, game_id TEXT, metadata TEXT NOT NULL, created_at TEXT NOT NULL, attempts INTEGER NOT NULL DEFAULT 0, next_attempt_at TEXT)');
      await db.execute(
          'CREATE TABLE content_versions (content_id TEXT PRIMARY KEY, version INTEGER NOT NULL, checksum TEXT NOT NULL, file_path TEXT NOT NULL, installed_at TEXT NOT NULL)');
    });
  ...
  Future<String> get installationId async {
    final rows = await db
        .query('settings', where: 'key = ?', whereArgs: ['installation_id']);
  ...
  Future<void> increment(String key) async => db.transaction((txn) async {
        final rows =
            await txn.query('stats', where: 'key = ?', whereArgs: [key]);
        final value = rows.isEmpty ? 1 : (rows.first['value']! as int) + 1;
        await txn.insert('stats', {'key': key, 'value': value},
            conflictAlgorithm: ConflictAlgorithm.replace);
      });

  Future<Map<String, int>> stats() async => {
        for (final row in await db.query('stats'))
  ...
  Future<List<Map<String, Object?>>> pending({int limit = 50}) =>
      db.query('sync_queue', orderBy: 'created_at', limit: limit);
  ...
  Future<void> acknowledge(Iterable<String> ids) async {
    ...
    await db.delete('sync_queue',
        where: 'id IN ($marks)', whereArgs: ids.toList());
  }
  ...
  Future<List<Map<String, Object?>>> installedContent() =>
      db.query('content_versions', orderBy: 'installed_at DESC');
